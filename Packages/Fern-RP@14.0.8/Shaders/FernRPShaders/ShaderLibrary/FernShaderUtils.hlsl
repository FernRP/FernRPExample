#ifndef UNIVERSAL_FERNSHADERUTILS_INCLUDED
#define UNIVERSAL_FERNSHADERUTILS_INCLUDED
...
float DepthSamplerToLinearDepth(float positionCSZ, float ProjectionParamsY, float ProjectionParamsZ)
{
    if(unity_OrthoParams.w)
    {
        #if defined(UNITY_REVERSED_Z)
        ...
        return lerp(ProjectionParamsY, ProjectionParamsZ, positionCSZ);
    }else {
        return LinearEyeDepth(positionCSZ,_ZBufferParams);
    }
}
